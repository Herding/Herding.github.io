...
马克可夫性质：$ P(S_{t+1} | S_{t}) = P(S_{t+1} | S_{1}, \cdots, S_{t}) $ 
...
收益函数：$ R_{s} = E \left [ R_{t+1}|S_{t} = s \right ] $ 

从某个时间戳开始的总回报：$G_{t} = \sum^{\infty}_{k = 0} \gamma^k R_{t+k+1}$
...
在MDP中的策略(policy)：$\pi(a| s) = P(A_{t} = a | S_{t} = s)$
