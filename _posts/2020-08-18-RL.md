...
马克可夫性质：$P(S_{t+1}|S_t) = P(S_{t+1}|S_1, \cdots, S_t)$

| MP       | MRP                  | MDP                    |
| -------- | -------------------- | ---------------------- |
| $<S, P>$ | $<S, R,  \gamma, P>$ | $<S, A, R, \gamma, P>$ |

#### 智能体如何根据短期回报(reward)定义长期收益：状态价值函数$v$或动作状态价值函数$q$

收益函数：$R_s = E[R_{t+1}|S_t = s]$

从某个时间戳开始的总回报：$G_t = \sum^{\infin}_{k = 0} \gamma^k R_{t+k+1}$
...
在MDP中的策略(policy)：$\pi(a|s) = P(A_t = a|S_t = s)$
...
Tabular Q learning：$Q(S, A) \larr Q(S, A) + \alpha [\underbrace { \overbrace{R + \gamma \max_a' Q(S', a')}^{\text{expected Q}} - Q(S, A)}_{\text{TD error}}]$
